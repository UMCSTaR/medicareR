...
#' name is "Denom_MBSF"
#' @param george_file: where the BG folder located at on maize. You may use the `computer_config`
#'    to help you detect.
#'
#'
...
mk_data_dir_path <- function(data_root = "sample",
  ...
  # original medicare locations
  wd$medicare_data_file <- "/original_medicare_selected_vars/data"
  # where the std medicare data will be saved to location
  wd$medicare_std_data_file<-"/standardized_medicare_data_using_R/std"
  ...
  if (data_root == "sample") {
    wd$src_data_root <-
      paste0(wd$george_file, wd$medicare_data_file, "/sample/", medicare_file, "/")
    wd$std_data_root <-
      paste0(wd$george_file, wd$medicare_std_data_file, "/sample/")

    wd
  }
  else if (data_root == "full_data") {
    wd$src_data_root <-
      paste0(wd$george_file, wd$medicare_data_file, "/", medicare_file, "/")
    wd$std_data_root <-
      paste0(wd$george_file, wd$medicare_std_data_file, "/full_data/")
